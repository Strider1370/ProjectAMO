...
- `frontend/src/features/weather-overlays/lib/weatherOverlayModel.js` -> weather overlay derived model for timeline, SIGWX history/filter state, advisory panel data, badge counts, legend labels, exact-frame WISSDOM selection, and mutually exclusive MAPLE QPF forecast selection.
- `frontend/src/features/weather-overlays/lib/weatherOverlayLayers.js` -> MET panel layer definitions, weather overlay source/layer ownership IDs, static weather overlay installation, and radar/satellite/SIGWX/advisory/lightning/WISSDOM/QPF Mapbox sync helpers.
...
- `frontend/src/features/weather-overlays/lib/echoTopLayers.js` + `useEchoTopOverlay.js` + `EchoTopCard.jsx` -> 재산출 Echo Top image source/layer lifecycle, click point lookup with stale-response guards, and the detail card. The card always states `재산출 · 18 dBZ · MSL`, whether the value is interpolated or a conservative beam-centre floor, and flags partial site coverage or a frame older than the selected time. Frame selection matches the radar layer (nearest previous), and collection delay is pinned to the radar's, so the two layers appear and disappear together.
- `frontend/src/features/weather-overlays/lib/wissdomLayers.js` + `qpfLayers.js` -> own exact-frame KMA WISSDOM and MAPLE QPF image source/layer lifecycle; QPF replaces observed radar/WISSDOM on forecast ticks and both restore safely after style reloads.
...
- `frontend/src/features/route-briefing/RouteWeatherLegTable.jsx` -> display-only desktop table/mobile-card renderer for `sections.enroute.legs` plus static procedure summaries: SID stays separate while STAR+IAP are merged into one arrival row with combined distance and hazards. Reports per-leg weather and source status without route matching or performance calculations; only ordinary en-route leg rows request map/profile highlighting.
...
- `backend/src/airport-model-comparison/{kim,open-meteo,gfs,lifecycle}.js` -> 기존 KIM 격자 재사용, Open-Meteo 실행 고정·ICON 보완 자료 검증, NOMADS GFS 수집을 담당한다. 해외 모델은 공개 예상 시각부터 1시간 간격의 세 정기 슬롯에서 미완료 자료를 수집한다. EC 구간은 해당 공항의 완전한 다른 모델 실행을 기준으로 이동한다. `backend/src/parsers/gfs-grib2-parser.js`는 지원 GRIB2 격자·시간·단순 패킹만 검증·판독한다. 수집은 실황 루트에 쓰고, 읽기는 활성 뷰를 따른다.
...
- `backend/src/lib/echo-top-grid.js` -> the 1 km Echo Top composite grid. It is the HSR radar grid at stride 2, so the published Echo Top image shares the radar layer's projection, bounds and pixel dimensions exactly.
- `backend/src/processors/echo-top-{model,store,processor}.js` -> **재산출 Echo Top**: 4/3-earth beam geometry, the 18 dBZ column solver (linear crossing when an upper bracket exists, conservative beam-centre floor otherwise — never extrapolated), per-site resampling with azimuthal fan filling, multi-site merge keeping the higher top, binary/WebP encoding, atomic publication with per-site status, retention, and a startup backfill. Not a KMA product: ProjectAMO derives it from raw site volumes, and it is independent of max reflectivity, echo height, satellite cloud top (CTPS) and convective initiation (CI), which come from different sources and mean different things.
- `backend/server.js` -> serves Echo Top metadata and a point API (FL / ft MSL / interpolated-vs-floor / contributing site) while blocking the server-only Echo Top binary, which carries per-site derived detail.
