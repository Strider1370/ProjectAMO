...
- `frontend/src/features/route-briefing/RouteWeatherLegTable.jsx` -> display-only desktop table/mobile-card renderer for `sections.enroute.legs` plus collapsible SID/STAR/IAP procedure summaries; reports per-leg weather and source status without route matching or performance calculations, and only procedure summary rows request map/profile highlighting.
