...
if [ "${PROJECTAMO_DEPLOY_LOCK_TEST:-}" = "1" ]; then
  ...
  case "${candidate_lock_file}" in
    /tmp/projectamo-deploy-lock-test.[[:alnum:]][[:alnum:]][[:alnum:]][[:alnum:]][[:alnum:]][[:alnum:]]/deploy.lock)
      ...
      ;;
    *)
      echo "[deploy-full] invalid test deployment lock path" >&2
      exit 2
      ;;
  esac
fi
...
echo "[deploy-full] restarting pm2 app from ecosystem.config.cjs..."
pm2 restart ecosystem.config.cjs --update-env
pm2 save

echo "[deploy-full] configuring pm2 log rotation..."
...
echo "[deploy-full] verifying process options..."
...
if [ -n "${expected_opts}" ]; then
  ...
  if [ "${actual_opts}" = "${expected_opts}" ]; then
    echo "[deploy-full] NODE_OPTIONS applied: ${actual_opts}"
  else
    echo "[deploy-full] NODE_OPTIONS mismatch — 파일: '${expected_opts}' / 실제: '${actual_opts}'" >&2
    exit 1
  fi
fi
...
echo "[deploy-full] recording successful deployment..."
