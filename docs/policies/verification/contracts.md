...
| `surface-pressure` | KIM PRMSL raster, MET toggle, NWP timeline, timestamp disclosure | desktop, iPad landscape | seven-frame key-free KIM manifest and WebP fixtures | `frontend/verification/contracts/surface-pressure.spec.mjs` | frontend | pending — fixture contract required before release |
| `my-map` | `MyMapPanel.jsx`, `useMyMap.js`, `folderView.js`, `myMapStore.js`, `Sidebar.jsx` 내 지도 항목, `useRouteBriefing.js` 지도파일 거부 | desktop, iPad landscape (모바일은 `MobileMapOverlay`에 진입점이 없어 skip — 1판 범위 밖) | 커밋된 `test/fixtures/my-map/folders.kmz`·`folders.kml`(폴더 3·지점 1·선 1·면 1); 기상 자료 없음 — 레이어 순서는 자료가 필요 없는 슬롯 배정으로 확인 | `frontend/verification/contracts/my-map.spec.mjs` | frontend | active — passed 2026-08-14 (desktop 8/8, iPad landscape 8/8, mobile 8 skipped) |
