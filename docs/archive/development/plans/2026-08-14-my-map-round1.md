...
**Spec:** `docs/archive/development/specs/2026-08-14-my-map-design.md`
...
- [ ] **Step 5: 눈으로 확인 (계약을 쓰기 전에)**

**이 단계를 건너뛰지 않는다.** Task 6·7·8은 지도 SDK와 화면에 직접 붙는 배선이라 단위 시험이 없다. 여기서 안 보면 Task 9의 계약 7건을 다 쓴 뒤에야 Task 6의 실수를 알게 된다 — 그때는 계약이 틀린 건지 구현이 틀린 건지 가리는 일부터 해야 한다.

[dev-server 절차](../../../operations/dev-server-and-capture.md)에 따라 띄운다:

```bash
ss -ltnp 2>/dev/null | grep -E ':3001|:5173' || echo "포트 비어 있음"
cd /home/john_doe/ProjectAMO && npm run dev:serve
```

서버가 준비되면 사용자에게 아래를 넘기고, **돌아온 값을 그대로 기록한다.** 브라우저 조작은 사람이 훨씬 빠르다.

> `http://127.0.0.1:5173` 을 열고 확인해주세요.
>
> 1. 왼쪽 아이콘 막대에 **내 지도**가 항공정보 아래에 있나요?
> 2. 눌렀을 때 설명 문구와 **파일 올리기**가 보이나요?
> 3. `/mnt/c/Users/Jond Doe/Downloads/맥케이 비행지도 ver.230729.kmz` 를 올려주세요.
>    - 지도에 도형이 뜨고 **화면이 그 파일 전체에 맞춰 움직이나요?**
>    - 폴더 목록이 **접힌 상태로 101줄**인가요? (맨 위가 `RKTA TAEAN`)
>    - `RKTA TAEAN` 왼쪽 **▸**를 누르면 하위 7개가 펼쳐지나요?
> 4. 찾기 상자에 `RKTA` 를 쳐주세요 — 맞는 폴더만 남나요? 스크롤해도 상자가 붙어 있나요?
> 5. 아무 폴더나 체크를 끄면 지도에서 그 도형이 사라지나요?
> 6. `RESTRICTED/MOA AREA` 의 **여기로**를 누르면 그 구역으로 이동하나요?
> 7. **기상정보 → 레이더**를 켜주세요. 레이더 그림이 이용자 지도 **위에** 오나요? (비 없는 곳에서는 아래 지도가 비쳐야 정상)
> 8. **기상정보 → 지형 근접**을 켜주세요. 지형 색칠이 이용자 지도 **아래**로 가나요?
> 9. 브라우저를 완전히 닫았다 다시 열어 **내 지도**를 열면 — 파일 목록은 남아 있고 **체크는 꺼져** 있나요?

**기록할 값:** 폴더 줄 수, 표시까지 걸린 느낌(빠름/3초쯤/느림), 7·8번의 위아래 순서, 9번 결과.

7번이나 8번이 틀리면 Task 6의 슬롯·순서 처리를 고친다. 나머지가 틀리면 해당 태스크로 돌아간다 — **여기서 임시로 덧대지 않는다.**

서버는 Task 9의 계약 실행 전에 끈다(`dev:contract`는 포트를 직접 관리한다).
...
세 겹이고, 각 겹이 앞 겹이 못 잡는 것을 잡는다.

| 언제 | 무엇을 | 못 잡는 것 |
| --- | --- | --- |
| 태스크 1~5 매번 | `node --test` — 계산이 맞는지 | 화면·지도에 실제로 붙었는지 |
| **태스크 8 끝 (Step 5)** | **사람이 눈으로** — 실제 파일로 한 번 | 다음에 누가 깼을 때 |
| 태스크 9 | 계약 7건 — 자동으로 되풀이 | 큰 파일에서만 나는 문제 |
| 태스크 9 끝 | 맥케이 파일로 마무리 | — |

계약이 작은 시험용 파일(700바이트)을 쓰는 이유는 빠르고 결과가 일정해야 하기 때문이다. 진짜 문제는 1.8MB에서 나오므로 맨 끝에 실제 파일로 한 번 더 본다.

## 완료 기준

- `cd frontend && npm test` 전부 통과 (옮겨온 22개 + 신규 31개)
- 태스크 8 Step 5의 눈 확인 9항목이 전부 통과 (특히 7·8번 위아래 순서)
