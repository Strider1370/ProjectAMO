...
- `API_OPERATION_REGISTRY` owns `{ id, label, provider, collectorType | null, dataHealthKeys, callContract, credentialCategory, apiHub, requestPolicy: { timeoutMs, maxAttempts, allowedOverrides }, match(url) }`. `maxAttempts` is the total physical HTTP attempts, including the initial request. `dataHealthKeys` maps each non-on-demand API to the existing data-health rows it updates; it is empty only for `{ kind: 'on_demand' }`. `callContract` is exactly `{ kind: 'collector' }`, `{ kind: 'cron', expression, timezone, quiet? }`, `{ kind: 'conditional', label }`, or `{ kind: 'on_demand' }`. `resolveApiOperation({ id, url })` requires one exact registered operation and requires an explicit id to match the resolved URL operation; `assertApiOperationRegistry()` rejects duplicate ids, ambiguous matchers, invalid data-health keys, missing data keys for non-on-demand APIs, invalid API Hub categories/policies, missing labels, an unresolved collector contract, and invalid cron/quiet shapes.
...
- Modify: `frontend/src/features/admin/screens/ApiUsageScreen.jsx`
...
- Create: `docs/archive/development/status/2026-08-31-collector-execution-observability.md`
...
```bash
git add docs/archive/development/status/2026-08-31-collector-execution-observability.md
