...
`docs/archive/development/status/typhoon-track-and-route-warning.status.md`에 한 페이지 이내로 남긴다: 완료 범위, 미측정으로 남긴 것(발표 게시 지연, 비대칭 방향 규칙 표본 3개), 실제 태풍 발생 시 할 일.

- [ ] **Step 6: 커밋**

```bash
git add docs/archive/development/status/typhoon-track-and-route-warning.status.md
