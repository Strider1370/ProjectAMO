...
**Spec:** `docs/archive/development/specs/2026-06-28-workflow-constitution-redesign-design.md`
...
## Task Packet
Meaningful work lives in `docs/archive/development/{specs,plans,status}/`. Spec = what/why (immutable). Plan = how (immutable). Status = cross-session handoff (mutable, under one page). Chat history is not the source of truth — read the packet first.
...
Run: `rg -i "codex" --glob '!docs/archive/development/archive/**' --glob '!backend/src/parsers/**' --glob '!docs/archive/development/specs/2026-06-28-*' --glob '!docs/archive/development/plans/2026-06-28-*'`
...
`docs/archive/development/status/workflow-constitution-redesign.status.md`에 완료 기록(Resume Point, Verified, Deviations). 그 후:
```bash
git add -A
