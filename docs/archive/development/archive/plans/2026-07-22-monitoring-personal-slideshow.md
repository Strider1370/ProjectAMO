...
**Spec:** docs/archive/development/specs/2026-07-22-monitoring-personal-slideshow.md
...
- [ ] Step 4a: Track the existing 719px phone-task breakpoint (matchMedia) in MonitoringPage. While it matches, force status to off regardless of stored configuration: skip the schedule/interval effects, keep any in-progress rotation stopped, and restore the live slide. A resize into the mobile range during active rotation must trigger the same Step 4 stop path (FR-014).
...
- [ ] Step 4a: When the Task 3 Step 4a mobile-breakpoint flag is on, render the 화면 전환 tab as unavailable (no enabled switch, target, interval, schedule, file, preview, or stop controls reachable) in both the inline and modal Settings surfaces, per FR-014.
- [ ] Step 5: Verify — manually open the inline settings surface and the modal surface in the fixed-data app, change each slideshow field in one surface, and confirm the other shows the same value without reopening the page. Stop from either surface, reload, and confirm it remains disabled. Resize below 719px and confirm the tab becomes unavailable and any active rotation stops.
...
- Modify: docs/archive/development/status/monitoring-personal-slideshow.status.md
...
- [ ] Step 5a: Add a Playwright contract at a 719px-or-narrower viewport that confirms the 화면 전환 tab controls are unavailable, and one that starts rotation at desktop width then resizes below 719px to confirm rotation stops and the live view is restored (FR-014, SC-012).
