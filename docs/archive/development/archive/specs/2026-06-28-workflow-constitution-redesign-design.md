...
- `docs/archive/development/{specs,plans,status}/*`, `docs/archive/development/archive/*` 과거 기록 — 보존(archive의 codex 잔재는 의도적 보존)
- `docs/research/*`, `docs/archive/development/specs/refs/*` 참고자료 — 보존
...
1. `.codex/` 디렉터리 부재. **"codex" 문자열 0건** — 검색 범위: 저장소 전체에서 `docs/archive/development/archive/`(과거 기록)와 `backend/src/parsers/*.js`(decodeXml 오탐) 제외. 대상에 전역·프로젝트 CLAUDE.md, Architecture.md, README.md, dev-server-and-capture.md 포함
