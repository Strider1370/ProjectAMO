...
- Archive completed or abandoned status files under `docs/archive/development/status/archive/`; review the currently active files before migration.
...
- Approved specifications are stored in `docs/archive/development/specs/`.
- Approved implementation plans are stored in `docs/archive/development/plans/`.
- Work that meets the long-context trigger creates a one-page mutable status file in `docs/archive/development/status/` before implementation begins.
...
- On completion or explicit abandonment, the implementation plan and status file record that state; the status file is then moved to `docs/archive/development/status/archive/` and is never deleted as routine cleanup.
