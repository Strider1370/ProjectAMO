...
## 0. 리서치 근거
`docs/archive/development/specs/refs/2026-06-27-efb-inputs-{foreflight-garmin,icao-pro,skydemon-ux}.md` 3종. 공통 결론: 빈칸 시작 금지(auto-fill→override), 속도=TAS, ETA는 파생, 고도 FL/ft+VFR힌트, 시간 Zulu 권위+빠른입력. 단 EFB는 항공기 DB로 자동채움 → 우리는 DB가 없으므로 **사용자 입력+저장**으로 대체.
