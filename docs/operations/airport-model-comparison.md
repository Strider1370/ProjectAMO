...
KIM, GFS, ICON은 실제 실행의 F000~F012를 쓴다. ECMWF는 실제 `run_at`을 유지하고 peer 최신 실행과 같은 유효시각을 덮도록 13개 forecast hour를 이동한다. 따라서 EC 00Z와 peer 06Z 조합은 EC F006~F018이 정상이다. `last-attempt.json`에는 대상 실행, 공항별 window, 성공·재사용·실패 공항, 오류와 수집 종료 이후의 다음 정기 점검 시각이 남는다.

## 해외 모델 수집 시각

10분 상시 점검 대신 KIM처럼 공개 예상 시간대에 정기 수집을 배치한다. 각 실행에 첫 시도와 1시간 간격 재시도 두 번을 둔다. 아래는 00 UTC 실행 기준이며 06/12/18 UTC 실행에는 각각 6/12/18시간을 더한다. 날짜를 넘는 재시도도 포함한다.

| 모델 | 첫 시도 UTC | 재시도 UTC |
|---|---|---|
| ICON | 04:40 | 05:40, 06:40 |
| GFS | 06:10 | 07:10, 08:10 |
| ECMWF | 07:40 | 08:40, 09:40 |

완전한 실행·구간은 외부 호출을 생략한다. 세 번째 정기 시도 후에도 실패하면 마지막 성공 자료를 유지하고 다음 실행의 첫 슬롯에서 다시 확인한다. 서버 시작 시 미완료 자료 복구, 관리자 수동 수집, peer 실행 변경에 따른 EC 구간 재평가는 정기 슬롯과 별도로 유지한다. 따라서 세 번은 정기 수집 슬롯 수이며 수동·복구 호출을 포함한 절대 요청 제한은 아니다.

실제 공개 여부는 계속 제공자 메타·파일로 검증한다. EC/ICON의 메타 가용시각 이후 10분 여유는 서버 간 반영 지연을 위한 조건이며 정기 재시도 간격과 다르다. 관리자 화면의 다음 점검은 현재 UTC cron으로 계산하므로 과거 시도 파일에 남은 예전 10분 주기를 표시하지 않는다.
