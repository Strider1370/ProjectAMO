...
기관에 가입하지 않은 사용자와 비로그인 사용자는 `/lounge`의 **미리보기 체험**에서 `/lounge/preview/home`으로 들어간다. 가입된 기관이 하나이면 기존처럼 바로 해당 기관에 진입하므로 미리보기 URL을 직접 열 수 있다.

체험은 실제 기관과 분리된 브라우저별 공간이다. 예시 비행 편집·공유, 문서/파일 등록, 합동 브리핑 및 지도 조작을 사용할 수 있다. 상단 **초기화**로 자신의 예시만 복원하고 **종료**로 메인 지도에 돌아간다. 미리보기 기상 브리핑에서 저장한 개인 경로도 체험 공간에만 남는다.

체험 데이터는 2시간 또는 서버 재시작 후 만료된다. 예시 입력 데이터는 `backend/src/organizations/preview-samples.json`과 합성 PDF에서 생성한다. 실제 기관 DB에서 예시를 가져오는 기능은 없다. 지도와 브리핑 기상은 서버가 보유한 실제 자료이며 오래된 자료/누락 상태는 그대로 표시한다. 로컬 DB의 예시를 수정해도 공개 미리보기 예시가 자동으로 바뀌지 않는다.
