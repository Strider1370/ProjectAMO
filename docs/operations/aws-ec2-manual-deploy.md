...
bash는 실행 중인 스크립트 파일을 읽어 가며 돈다. 그런데 `git pull`이 바로 그 파일을 바꿔치기하므로,
다시 시작하지 않으면 **그 실행은 끝까지 옛 내용으로 돈다** — 배포 절차를 고쳐도 이번이 아니라
"다음 배포부터" 적용된다. 2026-08-19에 이것 때문에 새로 넣은 안전망이 안 걸린 채로 옛 방식이
돌아 사이트가 다시 404로 내려갔다. 지금은 pull 직후 `exec bash "$0"`로 새 내용으로 재시작한다.
...
Linux `renameat2`의 같은 파일시스템 원자적 디렉터리 교환으로 반영한다. 빌드가 죽으면 기존 화면이 그대로 서빙되고,
