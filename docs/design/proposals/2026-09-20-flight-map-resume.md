...
2026-09-21 독립 검증에서 스타일·순서·메모 왕복과 부분 내보내기 집계 문제를 재현하고 보완했다. 최신 수정 내용과 검증 결과는 [4단계 검증 기록](2026-09-21-flight-map-review.md)을 먼저 읽는다.
...
1. **기관 공유 완료 — 2026-09-21 갱신.** 기관 API·서버 mount·공유창·기관 목록·새 버전 명시 적용·개인 사본·공유 중단을 연결했다. 실제 두 로그인 계정의 패널에서 저장 실패/지연, 지도 자료의 적용 전후, 권한 회수 후 숨김을 검증했다(FM-14~15). [5단계 기록](2026-09-21-flight-map-sharing-progress.md)의 「화면 통합 완료」가 최신이다. 다음 작업은 아래 작성 전 범위와 모바일/시안 대조다.
2. **작성·그룹 보완 진행 — 2026-09-21 갱신.** 면/원 완료·전체 이동 commit·꼭짓점 추가/삭제·반경 수치·undo/redo·그룹 DnD·좌표 일괄 기본 흐름을 브라우저에서 확인했다. 첫 점 완료·항목 드롭 중복·검색 정렬 오류를 수정하고, 새 그룹으로 묶기·순서 메뉴·자동 스크롤·접힌 그룹 펼침·삭제 개수 안내도 검증했다. 최신 근거와 남은 입력/이탈/경로 guard 검증은 [작성·그룹 보완 기록](2026-09-21-flight-map-authoring-progress.md)을 따른다(FM-05~08, 18).
3. **지도 관리·파일 변환 보완 완료 — 2026-09-21 갱신.** 일반 개인 지도 복제와 현재 편집 중 지도에 KML/KMZ 추가를 연결했다(FM-12, 16). 추가는 기존 항목과 원본을 보존하고 한 번의 undo/redo로 취소할 수 있다. 파일 드롭·취소·재열기·원본 메타데이터·사용자 메모, 새 편집본·개인 복제·독립 삭제를 데스크톱 계약으로 확인했다. 변환 중 이름 덮어쓰기도 수정했다. 최신 근거는 [작성·그룹 보완 기록](2026-09-21-flight-map-authoring-progress.md)에 있다.
4. **모바일·반응형·시안 대조와 FM-01~21 증거 갱신.** Pixel 5 Chromium에서 파일 열기·폴더 표시 전환·재열기를, iPad Pro 11 가로 Chromium에서 같은 흐름과 배경지도 두 번 전환 뒤 선택/숨김 복원을 확인했다. 실제 AIRAC 공역의 `DistVertUpper_Val: "5000"`은 개인 편집본·사본에서 보존됨을 확인했고, 기관 API·화면 계약은 같은 원본 `source` 전체 불변성을 확인한다. 작은 노트북, 모바일 진입·종료 후 기존 지도 도구 복구와 시안 대조는 남아 있다. 실제 iPad Safari는 장비가 없으면 미검증으로 명시한다(FM-20).
