...
2. 가우시안(σ 1.5°)으로 세게 평활화한다. H/L도 이 장에서 찾는다(2026-09-23 변경, 아래 "2026-09-23 H/L 판정 변경").
...
2026-09-22 화면 검토에서 σ 0.5° 등압선은 격자 요철을 따라 찌그러지고 작은 고리가 생겨, 날씨누리 수치모델 화면처럼 완만하게 바꿨다. 그 결과 약한 H/L은 둘러싸는 등압선 없이 기호만 보일 수 있었다(날씨누리도 같다). 2026-09-23부터는 H/L도 이 장에서 닫힌 등압선으로 찾으므로 기호에는 항상 닫힌 등압선이 있다.
...
| 닫힌 중심 판정 | 창 테두리 최솟값이 1 hPa 높은지(대용) | ~~중심에서 채워 나가 대권거리 6° 안에서 1 hPa 닫힘 확인(TempestExtremes)~~ 2026-09-23 삭제. 그린 등압선이 닫혀 있는 것 자체를 닫힘의 근거로 삼는다 | WPC 매뉴얼 H/L 정의 |
...
- 기본은 바람깃이다(2026-09-22 결정 변경: 한때 애니메이션 기본이었다). 애니메이션은 버튼으로 켠다.
...
- 프론트엔드: `useKimSurfaceChart.js`(훅), `surfaceChartModel.js`, `surfaceChartLayers.js`, `surfaceChartLegend.js`. MET 패널 `강수` 타일과 `수치모델` 제목 옆 바람 표시 전환 버튼(기본 바람깃), 검색 등록, 범례, `기상자료 시각` 항목, 타임라인 미래 눈금.
...
## 2026-09-23 H/L 판정 변경

2026-09-23 06UTC 런 +9시간(09/24 00:00 KST) 화면에서 두 문제가 보였다. 닫힌 등압선이 그려졌는데 기호가 없는 계가 있었고(그린 가장 안쪽 닫힌 등압선 6개 중 3개), 기호가 닫힌 등압선 안에서 한쪽으로 치우쳤다.

원인은 등압선(σ 1.5°)과 H/L(σ 0.5°)이 서로 다른 장에서 나온 것이다.
- 약하게 평활한 장에서는 계의 모양이 달라, 화면에는 닫혀 보이는 계가 판정에서 빠진다(중국 서부 L 1001).
- 6° 안 1 hPa 닫힘(TempestExtremes)은 저기압 추적용 기준이라, 넓고 평평한 고기압이 빠진다(일본 동쪽 H 1021, 깊이 2.4 hPa인데 6° 안에서 1 hPa가 닫히지 않음).
- 바닥이 평평한 계는 약하게 평활한 장의 극값이 선 모양과 어긋난다(L 1015가 고리 지름의 31%만큼 치우침).

변경한 규칙은 하나의 장, 하나의 정의다.
1. 등압선과 같은 장(σ 1.5°), 같은 규칙(2 hPa, 1.5° 미만 고리 제외)으로 닫힌 등압선을 뽑는다.
2. CycloneDetector처럼 가장 안쪽 닫힌 등압선을 후보로 삼고, 면적(1만 km²) 미만은 바깥 등압선으로 올린다.
3. 기호는 그 등압선 안 극값 자리에 둔다. 종류는 등압선 값보다 안쪽이 낮으면 L, 높으면 H다.
4. 중심기압은 그 등압선 안 원자료(1/12°) 극값이다.
5. TempestExtremes 6° 닫힘 확인과 σ 0.5° 장은 없앤다.

결과. 같은 시각 기호는 H 1027, H 1021, L 1001, L 1006, L 1012, L 1015로, 그린 가장 안쪽 닫힌 등압선 6개와 하나씩 맞는다. 기호와 고리 무게중심의 거리(고리 크기 대비)는 L 1015 31%→4%, L 1001 11%→3%, H 1021 16%→10%, L 1012 29%→20%다. L 1012는 1014 hPa 고리가 동서로 길고 그 장의 최저점이 동쪽에 있어 남는 치우침이다.

대가. 닫힌 등압선 없이 기호만 있던 약한 극값은 표시하지 않는다(이 시각의 H 1017, 96.3°E 38.8°N, 2026-09-22 시험의 동해 H 1020). 서해 기압능(최고 1018.8 hPa, 북쪽 고기압과 0.3 hPa 차이)처럼 닫힌 선이 없는 약한 능·골은 등압선 모양으로 읽는다. 2026-09-22 시험의 "CD+TE 7개" 기대값은 이 변경 전 기준이다.

테스트: 그린 등압선이 닫히는 넓은 고기압은 H, 닫힌 등압선이 없는 극값은 기호 없음, 기호는 그린 장의 극값 자리이고 중심기압은 원자료 극값.
