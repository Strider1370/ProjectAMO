...
SQLite에 다음 세 종류의 자료를 둔다.

1. `collector_runs`: 시작·종료·skip·missed의 불변 실행 이력. 수집기, 예정·시작·종료 시각, outcome, duration, 안전한 요약, 오류 코드를 저장한다. `succeeded` 상세 이력은 **7일**, `degraded`·`failed`·`skipped`·`missed` 이상 이력은 **180일** 보관한다.
2. `collector_daily_summary`: 수집기·일자별 성공/부분 성공/실패/건너뜀/미실행 횟수, 최장 소요 시간, 첫·마지막 이상 시각만 저장한다. 상세 이력이 정리된 뒤에도 **180일** 동안 장기 추세를 제공한다.
3. `collector_state`: 수집기별 마지막 예정·시작·종료·outcome·missed 사건을 요약한 현재 상태. 서버 재시작 뒤 watchdog가 즉시 같은 사건을 중복 생성하지 않게 한다.

정리는 하루에 한 번만 원자적으로 수행한다. 행 삭제 뒤 SQLite WAL을 checkpoint하여 WAL 파일이 장기간 커지지 않게 하고, 정리 대상은 인덱스가 있는 수집기·시각 조건으로 제한한다. 따라서 수집량이 늘어도 실행 이력 저장 공간은 위 보존 기간을 넘겨 무한히 증가하지 않는다. `stats/latest.json`은 기존 성공률·공항별 통계의 호환성을 위해 유지하지만, 실행 이력의 단일 진실원은 SQLite다.
...
PM2 stdout/stderr도 무제한 파일로 두지 않는다. PM2 로그 회전을 설정해 파일이 10 MiB에 도달하거나 하루가 지나면 회전하고, 압축된 보관본은 최근 7개만 남긴다. 표준 출력과 오류 출력 모두 같은 상한을 적용한다. SQLite 이력은 콘솔의 검색 가능한 원인 기록이고, PM2 로그는 단기 운영 진단용 원본이라는 역할을 분리한다.
...
- 이력 보존 안내: 성공 상세 7일, 이상 상세과 일별 집계 180일
...
1. 실행 기록 모듈 단위 테스트: start→succeeded, failed, skipped, degraded, 성공 상세 7일·이상 상세와 일별 집계 180일 보존 정리.
...
8. 운영 검증: 배포 뒤 지상예보의 다음 정규 실행에서 서버 로그·SQLite 이력·관리자 콘솔 모두 같은 예정/시작/종료 시각과 `succeeded` 결과를 보이는지 확인한다. PM2 stdout와 stderr가 회전·압축되고 보관 파일 수가 상한을 넘지 않는지도 확인한다.

## 범위 밖

- PM2·EC2·AWS 외부 가용성 감시 및 자동 복구 (PM2 로그 회전은 포함)
