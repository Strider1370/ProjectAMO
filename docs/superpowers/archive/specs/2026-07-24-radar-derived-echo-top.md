...
- FR-005: 레이어는 독립 토글로 제공하며 기본 OFF다. ON일 때 표시 프레임은 **레이더 레이어와 동일한 선택 규칙**(선택 시각 이하의 가장 최근 프레임)을 따른다. 수집 지연도 레이더와 같게 맞춰 평상시에는 두 레이어의 시각이 일치하며, 한 수집 주기를 놓쳤을 때만 직전 프레임이 대신 표시된다. 그 경우 프레임이 선택 시각보다 과거임을 UI에 명시해야 한다.
  - **2026-07-26 개정.** 원래 요구는 "선택 시각과 정확히 맞는 프레임만 표시"였다. 그러나 수집 지연이 레이더 10분 / 에코탑 15분으로 달라, 실시간 보기의 기준 시각이 항상 레이더 최신 시각이 되면서 에코탑 프레임이 존재하지 않아 **레이어가 상시 숨겨지는** 문제가 실제 운영에서 확인됐다. 지연을 레이더와 같은 10분으로 맞추고(실측: QCD는 5분 지연으로도 전량 수신), 선택 규칙도 레이더와 통일했다. FR-002(수집 단계에서 다른 bucket의 파일을 그 프레임으로 발행 금지)는 그대로 유지되므로, 발행된 프레임의 시각은 언제나 실제 관측시각이다 — 대체가 일어나도 그 프레임의 진짜 시각이 표시된다.
...
| No frame at all | 발행된 프레임이 하나도 없으면 레이어를 숨기고 자료 없음을 표시한다. |
| Partial site coverage | 정상 사이트만 합성한다. 부분 커버리지임을 식별 가능하게 표시한다. |
| Late/stale file | 수집 단계에서 그 프레임으로 발행하지 않는다(FR-002). 표시 단계에서 직전 프레임으로 대체될 때는 그 프레임의 실제 관측시각을 표시하고, 선택 시각보다 과거임을 함께 밝힌다 — 이전 결과를 새 시각처럼 표시하지 않는다. |
