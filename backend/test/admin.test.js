...
test('admin endpoints require admin and return data', async () => {
  ...
  try {
    ...
    const metrics = await (await fetch(at(server, '/api/admin/metrics?range=24h'), getWith(cookie))).json()
    assert.equal(metrics.time.timestampStandard, 'UTC_ISO_8601')
    assert.equal(metrics.time.seriesFilesystem, 'root_filesystem')
    assert.equal(metrics.current.metricContract.cpu.metric, 'one_minute_load_average_normalized_by_logical_cpu_count')
    assert.ok(metrics.current.metricContract.filesystems.root.role)
    const traffic = await (await fetch(at(server, '/api/admin/traffic'), getWith(cookie))).json()
    ...
    assert.ok(health.rows.every((r) => r.provenance?.display && r.provenance?.liveCollection))
    assert.ok(health.rows.every((r) => r.stats?.executionCounters?.period?.kind === 'stats_lifetime_since_reset'))
    ...
    assert.equal(trends.measurement.businessDay, 'UTC_calendar_day')

    const watches = await (await fetch(at(server, '/api/admin/alert-watches'), getWith(cookie))).json()
    assert.equal(watches.evaluation.currentSelection.policy, 'one_earliest_route_per_user_within_configured_window')
    assert.ok(Array.isArray(watches.watches))
