...
export const CHAT_TOOLS = Object.freeze({
  ...
  get_airport_weather: { schema: z.object({
    ...
    description: 'Airport METAR/TAF/airport warnings. Current: hoursFromNow (default 1). Relative future ("3시간 뒤"): startsInHours, never compute its date yourself. Same previous time: usePreviousWindow=true only when the user refers to it; no stated time means current. Explicit dated local time: localWindow (server converts using displayTimezone); explicit UTC: window. Choose ONE time mode, unused fields null. includeRaw=false unless TAC requested. Not route icing/turbulence or SIGMET/AIRMET.' },
