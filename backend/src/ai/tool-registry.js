...
export const CHAT_TOOLS = Object.freeze({
  ...
  get_airport_weather: { schema: z.object({
    ...
    hoursFromNow: z.number().int().min(1).max(48).optional(), includeRaw: z.boolean().default(false),
  }).strict().refine((v) => [v.window, v.localWindow, v.usePreviousWindow, v.hoursFromNow !== undefined].filter(Boolean).length <= 1),
    description: 'Airport METAR/TAF/airport warnings. Current: hoursFromNow (default 1). Same previous time: usePreviousWindow=true. Explicit dated local time: localWindow (server converts using displayTimezone); explicit UTC: window. Choose ONE time mode, unused fields null. includeRaw=false unless TAC requested. Not route icing/turbulence or SIGMET/AIRMET.' },
  get_weather_advisories: { schema: AdvisoryInputSchema.safeExtend({ localAt: localInstant.optional(), localWindow: localWindow.optional() })
    ...
    description: 'Domestic SIGMET/AIRMET. For NOW leave all times null. Explicit local date/time: localAt or localWindow in displayTimezone, YYYY-MM-DDTHH:mm. Explicit UTC: at/window. ONE time mode only. types selects sigmet/airmet/both. No route/altitude assessment. Page with result_ref+cursor, no query filters.' },
  get_route_briefing: { schema: z.object({ context_ref: z.string().min(1).max(100) }).strict(),
    description: 'Read weather comparisons for a registered applied route OR a newly calculated plan_route result using its contextRef as context_ref. No arbitrary coordinates, altitude changes or fixture_id in app chat.' },
  get_briefing_detail: { schema: BriefingDetailInputSchema,
    description: 'Read a stored briefing section without recalculating. Use briefing_ref, section and optional cursor/limit (1–20). enroute contains a summary followed by individual enroute/procedure leg records, without coordinates. Follow nextCursor for remaining items; a partial page is not the full route.' },
  compare_route_altitudes: { schema: AltitudeComparisonInputSchema,
    description: 'Compare 2–5 distinct requested altitudes in feet on an existing briefing_ref. Uses the SAME captured weather/AIP/run/route/time. Preserve valid/input_only/input_invalid and data gaps. Whole-route weather comparison, not altitude recommendation or safety ranking. No coordinates or new time inputs.' },
})
