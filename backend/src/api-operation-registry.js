...
export const API_OPERATION_REGISTRY = Object.entries(API_HUB_ENDPOINTS).map(([id, label]) => ({ id, label, provider: 'KMA API Hub', collectorType: collectorById[id] || null, dataHealthKeys: health[id] || ['environment'], callContract: collectorById[id] ? { kind: 'collector' } : { kind: 'conditional', label: '수집 시' }, credentialCategory: category(id), apiHub: true, canonicalUrl: `https://apihub.kma.go.kr${canonicalPaths[id]}`, requestPolicy: apiHubPolicyFor(id), match: (url) => url.hostname === 'apihub.kma.go.kr' && pathMatchers[id](url.pathname) && (id !== 'radar_hsr' || url.searchParams.get('cmp') !== 'HCI') && (id !== 'radar_hci' || url.searchParams.get('cmp') === 'HCI') && (id !== 'kim_grid' || !isSurfaceChartKimRequest(url)) && (id !== 'kim_grid_chart' || isSurfaceChartKimRequest(url)) }))
  .concat([
    ...
    // authKey, but still must pass through the observable transport; otherwise a descriptor
    // can succeed while every image download is rejected as an unknown operation.
    { id: 'kma_radar_graphics_asset', label: '레이더 그래픽 이미지', provider: 'KMA API Hub', collectorType: null, dataHealthKeys: ['radar', 'hci', 'wissdom', 'qpf'], callContract: { kind: 'conditional', label: '레이더 그래픽 수집 시' }, credentialCategory: null, apiHub: false, canonicalUrl: 'https://apihub.kma.go.kr/data/radar-graphics.webp', requestPolicy: { timeoutMs: 30_000, maxAttempts: 1, allowedOverrides: ['signal'] }, match: (url) => url.hostname === 'apihub.kma.go.kr' && url.pathname.startsWith('/data/') },
    ...
    { id: 'open_meteo_air_quality', label: 'Open-Meteo 대기질', provider: 'Open-Meteo', collectorType: 'environment', dataHealthKeys: ['environment'], callContract: { kind: 'collector' }, credentialCategory: null, apiHub: false, canonicalUrl: 'https://air-quality-api.open-meteo.com/v1/air-quality', requestPolicy: { ...policy, timeoutMs: config.environment.timeout_ms }, match: (url) => url.hostname === 'air-quality-api.open-meteo.com' && url.pathname === '/v1/air-quality' },
