...
const apiHubPolicy = { timeoutMs: config.api.timeout_ms, maxRetries: config.api.max_retries, allowedOverrides: ['signal', 'maxRetries', 'retryDelayMs', 'skipApiHeader'] }

export const API_OPERATION_REGISTRY = Object.entries(API_HUB_ENDPOINTS).map(([id, label]) => ({ id, label, provider: 'KMA API Hub', collectorType: collectorById[id] || null, dataHealthKeys: health[id] || ['environment'], callContract: collectorById[id] ? { kind: 'collector' } : { kind: 'conditional', label: '수집 시' }, credentialCategory: category(id), apiHub: true, canonicalUrl: `https://apihub.kma.go.kr${canonicalPaths[id]}`, requestPolicy: apiHubPolicy, match: (url) => url.hostname === 'apihub.kma.go.kr' && pathMatchers[id](url.pathname) && (id !== 'radar_hsr' || url.searchParams.get('cmp') !== 'HCI') && (id !== 'radar_hci' || url.searchParams.get('cmp') === 'HCI') }))
  .concat([
...
export function assertApiOperationRegistry(registry = API_OPERATION_REGISTRY, collectors = COLLECTOR_REGISTRY) {
  ...
  for (const operation of registry) {
    if (!operation?.id || !operation.label || !operation.provider || typeof operation.match !== 'function' || !operation.canonicalUrl || ids.has(operation.id)) throw registryError('invalid_api_operation_registry')
    ...
    if (operation.callContract.kind === 'collector' && (!operation.collectorType || !collectorTypes.has(operation.collectorType))) throw registryError('unresolved_api_operation_collector')
    if (operation.callContract.kind === 'cron') {
      ...
      if (typeof operation.callContract.expression !== 'string' || typeof operation.callContract.timezone !== 'string') throw registryError('invalid_api_operation_contract')
    ...
    if (operation.callContract.kind === 'conditional' && !operation.callContract.label) throw registryError('invalid_api_operation_contract')
...
function cadenceLabel(expression) { const [minute, hour] = expression.split(' '); if (minute.startsWith('*/') && hour === '*') return `${minute.slice(2)}분마다`; if (minute.startsWith('*/') && /^\d+-\d+$/.test(hour)) return `${minute.slice(2)}분마다 (${hourRangeLabel(hour)})`; if (/^\d+(,\d+)+$/.test(hour)) return hour.split(',').map((value) => `${value.padStart(2, '0')}:${minute.padStart(2, '0')}`).join(', '); return `${minute}분 ${hour}시` }
