...
function apiHubPolicyFor(id) {
  if (['radar_wissdom', 'radar_qpf', 'radar_hsr', 'radar_hci'].includes(id)) return { timeoutMs: 30_000, maxRetries: 0, allowedOverrides: ['signal'] }
  if (id === 'asos_ceiling') return { timeoutMs: config.asos_ceiling.timeout_ms, maxRetries: 0, allowedOverrides: ['signal'] }
  if (id === 'radar_qcd') return { timeoutMs: config.radar_echo_top.timeout_ms, maxRetries: config.radar_echo_top.retry, allowedOverrides: ['signal'] }
  if (['radar_echo', 'sfc_vis'].includes(id)) return { timeoutMs: 30_000, maxRetries: 0, allowedOverrides: ['signal'] }
  ...
  if (id === 'lightning') return { timeoutMs: 30_000, maxRetries: 2, retryDelayMs: 3_000, allowedOverrides: ['signal'] }
  if (id === 'typhoon_now' || id === 'typhoon_list') return { timeoutMs: 15_000, maxRetries: 0, allowedOverrides: ['signal'] }
  if (['ground_forecast', 'mid_land', 'mid_ta'].includes(id)) return { timeoutMs: config.ground_forecast.timeout_ms, maxRetries: 0, allowedOverrides: ['signal'], transportFallback: groundForecastFallback }
...
const ALLOWED_OVERRIDES = new Set(['signal', 'headers', 'method', 'body', 'maxRetries', 'retryDelayMs', 'skipApiHeader'])
...
export function assertApiOperationRegistry(registry = API_OPERATION_REGISTRY, collectors = COLLECTOR_REGISTRY) {
  ...
  for (const operation of registry) {
    ...
    const requiredPolicyKeys = ['allowedOverrides', 'maxRetries', 'timeoutMs', ...(operation.requestPolicy?.retryDelayMs === undefined ? [] : ['retryDelayMs']), ...(operation.requestPolicy?.transportFallback === undefined ? [] : ['transportFallback'])].sort()
    const fallback = operation.requestPolicy?.transportFallback
    const validFallback = !fallback || (Object.keys(fallback).sort().join(',') === 'maxAdditionalAttempts,transport,trigger' && Number.isInteger(fallback.maxAdditionalAttempts) && fallback.maxAdditionalAttempts === 1 && Object.keys(fallback.trigger || {}).join(',') === 'causeCode' && fallback.trigger.causeCode === 'SELF_SIGNED_CERT_IN_CHAIN' && Object.keys(fallback.transport || {}).sort().join(',') === 'headers,kind,rejectUnauthorized' && fallback.transport.kind === 'https_request' && fallback.transport.rejectUnauthorized === false && Object.keys(fallback.transport.headers || {}).join(',') === 'User-Agent' && fallback.transport.headers['User-Agent'] === 'KMA-Weather-Dashboard/1.0')
    if (!operation.requestPolicy || policyKeys.length !== requiredPolicyKeys.length || !requiredPolicyKeys.every((key, index) => key === policyKeys[index]) || !Number.isFinite(operation.requestPolicy.timeoutMs) || operation.requestPolicy.timeoutMs <= 0 || !Number.isInteger(operation.requestPolicy.maxRetries) || operation.requestPolicy.maxRetries < 0 || (operation.requestPolicy.retryDelayMs !== undefined && (!Number.isFinite(operation.requestPolicy.retryDelayMs) || operation.requestPolicy.retryDelayMs < 0)) || !validFallback || !Array.isArray(operation.requestPolicy.allowedOverrides) || operation.requestPolicy.allowedOverrides.some((key) => typeof key !== 'string' || !ALLOWED_OVERRIDES.has(key))) throw registryError('invalid_api_operation_policy')
