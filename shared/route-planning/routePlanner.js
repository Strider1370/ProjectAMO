...
// One planner uses one immutable publication provider. No environment I/O here.
export function createRoutePlanner(provider) {
  const { loadNavdata } = provider
  ...
  async function buildVfrRoute({ departureAirport, arrivalAirport }) {
  ...
  async function buildBriefingRoute({ departureAirport, entryFix, exitFix, arrivalAirport, routeType, viaFixes = [] }) {
    const navdata = await loadNavdata()
    const departureId = normalizeIdent(departureAirport)
    const arrivalId = normalizeIdent(arrivalAirport)
  ...
  async function buildManualIfrRoute({ departureAirport, arrivalAirport, enroute, userWaypoints = [], routeType = 'ALL' }) {
    const navdata = await loadNavdata()
    const departureId = normalizeIdent(departureAirport)
    const arrivalId = normalizeIdent(arrivalAirport)
    const departure = navdata.airports[departureId]
  ...
  async function buildManualVfrRoute({ departureAirport, arrivalAirport, enroute, userWaypoints = [] }) {
    const navdata = await loadNavdata()
    const departureId = normalizeIdent(departureAirport)
    const arrivalId = normalizeIdent(arrivalAirport)
    const departure = navdata.airports[departureId]
  ...
  async function resolveNearestNavpoint(coordinates) {
  ...
  async function resolveMapInteraction({ coordinates, departureAirport, entryFix, exitFix, arrivalAirport, routeType, viaFixes = [] }) {
  ...
  async function parseRouteString(text, { departureAirport, entryFix, exitFix, arrivalAirport, routeType }) {
  ...
  function formatRouteString(routeResult) {
  ...
  async function canBuildBriefingRoutePath({ entryFix, exitFix, routeType }) {
    const navdata = await loadNavdata()
    const entryId = normalizeIdent(entryFix)
    const exitId = normalizeIdent(exitFix)
    const selectedRouteType = routeType ?? 'ALL'
  ...
  return { ...provider, buildVfrRoute, buildBriefingRoute, buildManualIfrRoute, buildManualVfrRoute, resolveNearestNavpoint, resolveMapInteraction, parseRouteString, formatRouteString, canBuildBriefingRoutePath }
}
