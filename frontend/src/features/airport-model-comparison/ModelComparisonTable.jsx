...
function CeilingCell({ cell }) {
  return <>{Array.isArray(cell.clouds) && cell.clouds.length > 0 ? <details className="mc-cell-disclosure"><summary aria-label={cell.text}><strong>{cell.text}</strong></summary><small>전/저/중/상 {cell.clouds.map(v => Number.isFinite(v) ? `${Math.round(v)}%` : v && typeof v === 'object' ? (v.amount || v.coverage || '—') : '—').join(' · ')}</small></details> : <strong>{cell.text}</strong>}<ConditionalCell text={cell.conditionText} /></>
}
  ...
  return (
    <div className="mc-table-scroll" tabIndex="0" aria-label={`${names[section]} 표 스크롤 영역`}>
      <table className="mc-table" aria-label={names[section]}>
        ...
        <tbody>{rows.map(row => <tr key={row.id}><th scope="row" aria-label={row.label} aria-describedby={section === 'ceiling' && row.cells.some(cell => cell?.method) ? `mc-method-${row.id}` : undefined}><span className="mc-series-key" style={{ '--series-color': row.color }} />{row.label}{section === 'ceiling' && row.cells.find(cell => cell?.method)?.method && <small id={`mc-method-${row.id}`} className="mc-method">{row.cells.find(cell => cell?.method).method}</small>}</th>{row.cells.map((cell, i) => <td key={times[i]} className={[selectedValidAt === times[i] && 'is-selected', cell?.status === 'observation_pending' && 'mc-cell--observation-pending'].filter(Boolean).join(' ')} title={cell?.detail ? [row.label, cell.valid_at, forecastSummary(cell)].join(' · ') : undefined} aria-label={cell?.status === 'observation_pending' ? `${row.label} ${timeLabels[i]} 관측 전` : undefined}>{cell?.status === 'observation_pending' ? <span className="mc-observation-pending">관측 전</span> : cell ? <>{section === 'wind' ? <WindCell cell={cell} /> : section === 'ceiling' ? <CeilingCell cell={cell} /> : section === 'temperatureRh' ? <TemperatureCell cell={cell} /> : <><strong>{cell.text}</strong><ConditionalCell text={cell.conditionText} /></>}</> : <span className="mc-missing">자료 없음</span>}</td>)}</tr>)}</tbody>
      </table>
    </div>
  )
}
