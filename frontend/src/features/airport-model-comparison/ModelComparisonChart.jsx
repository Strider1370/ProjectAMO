  ...
  return (
    <div className="mc-chart-wrap">
      <svg ref={chartRef} tabIndex="0" onFocus={event => showTime(event, focusedTime(), 'focus')} onBlur={closeDetail} onKeyDown={moveKeyboardTime} onPointerMove={event => { if (event.pointerType === 'touch') return; keepDetail(); const at = chartTime(event); if (at) setDetail({ at, x: event.clientX, y: event.clientY, mode: 'hover' }); else closeDetail() }} onPointerLeave={leaveDetail} onPointerDown={event => event.preventDefault()} className="mc-chart" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="group" aria-label={`모델별 ${unit} 추세 그래프`} aria-describedby={detail ? tooltipId : undefined}>
        ...
        {series.flatMap(s => s.points.map((point, i) => point.conditionText && Number.isFinite(Date.parse(point.at)) ? <path key={`${s.id}-condition-${i}`} d={`M${xAt(point.at) - 5},${PAD_Y - 8} l10,0 l-5,8 Z`} fill={s.color} tabIndex="-1" role="img" aria-label={`${s.label} 조건 ${point.at}, ${point.conditionText}`} {...pointEvents(point)}><title>{point.conditionText}</title></path> : null))}
      ...
      <div className="mc-legend">{series.map(s => <span key={s.id}><i style={{ '--series-color': s.color }} />{s.label}{s.points.every(p => !Number.isFinite(p.value) && !Number.isFinite(p.gust) && !Number.isFinite(p.secondary)) ? ' (자료 없음)' : ''}</span>)}{series.some(s => s.points.some(p => Number.isFinite(p.gust))) && <span>긴 점선·빈 점 돌풍</span>}{series.some(s => s.points.some(p => p.conditionText)) && <span>▼ TAF 조건 구간</span>}{secondaryUnit && <span>점선 {secondaryUnit}</span>}{ceilingChart && <span>▲ 10,000 ft 이상</span>}</div>
