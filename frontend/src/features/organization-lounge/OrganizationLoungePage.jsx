...
import { listOrganizations, organizationRequest } from './api.js'
...
function OrganizationPicker({ organizations, onSelect }) {
...
export default function OrganizationLoungePage() {
  ...
  useEffect(() => {
    ...
    listOrganizations({ signal: controller.signal }).then((result) => {
      ...
      if (!parseLocation().orgId && items.length === 1) {
        const id = items[0].organization?.id || items[0].id
        go(`/lounge/${encodeURIComponent(id)}`, true)
      }
  ...
  useEffect(() => { setData(EMPTY_DATA) }, [route.orgId, user?.id])

  const reload = useCallback(() => setRefreshKey((value) => value + 1), [])
  useEffect(() => {
    if (!route.orgId || !user || route.action === 'present') return
    const timer = window.setInterval(reload, 30_000)
    return () => window.clearInterval(timer)
  }, [route.orgId, route.action, user?.id, reload])
  useEffect(() => {
    if (!route.orgId || !user) { setLoading(false); return }
    const controller = new AbortController()
    let active = true
  ...
  }, [route.orgId, user?.id, refreshKey])
  ...
  const context = useMemo(() => ({ orgId: route.orgId, route, data, tz, user, navigate, reload }), [route, data, tz, user, navigate, reload])

  if (authLoading || (pickerLoading && !route.orgId)) return <div className="ol-loading" role="status"><LoaderCircle className="ol-spin" /> 기관 라운지를 불러오는 중…</div>
  if (!user) return <main className="ol-entry"><section className="ol-entry-card"><LogIn size={28} /><h1>로그인이 필요합니다.</h1><p>기관 자료는 구성원 권한을 확인한 뒤 제공합니다.</p><a className="ol-button" href="/">지도에서 로그인</a></section></main>
  if (!route.orgId) return <OrganizationPicker organizations={organizations} onSelect={(id) => go(`/lounge/${encodeURIComponent(id)}`)} />
  ...
  return <div className={`ol-shell ${sidebarExpanded ? 'sidebar-is-expanded' : ''}`}>
    ...
    <div className="ol-workspace">
      <header className="ol-topbar">
        ...
        <span className="ol-user"><UserRound size={17} />{user.display_name || user.username}</span>
      </header>
      ...
      <main id="organization-lounge-main" className="ol-main" tabIndex="-1">
        ...
        <Screen key={`${route.orgId}:${user.id}`} {...context} />
      </main>
    </div>
  </div>
}
