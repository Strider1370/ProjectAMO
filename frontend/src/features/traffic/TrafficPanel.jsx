...
export default function TrafficPanel({
  ...
  // 껍데기(데스크톱 서랍 / 모바일 시트)만 다르고 알맹이는 하나다.
  const body = (
  ...
  // 모바일은 다른 패널과 같은 하단 시트 — 지도가 위에 남아야 필터 효과를 보면서 조절할 수 있고,
  // 좌상단 레이어 버튼과도 겹치지 않는다.
  if (isMobile) {
    return (
      <MobileSheet open eyebrow="교통" title="항적 (ADS-B)" onClose={onClose} footer={footer}>
        {body}
      </MobileSheet>
    )
  }
  ...
  return (
    <div className="dev-layer-panel layer-drawer traffic-panel" aria-label="항적 필터">
      ...
      <div className="layer-drawer-footer">{footer}</div>
    </div>
  )
}
