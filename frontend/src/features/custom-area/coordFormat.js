...
// ── 표시용 역포맷: 십진수 → 문자열 (좌표 확인 도구용) ────────────────────────
const HEMI = { lat: ['N', 'S'], lng: ['E', 'W'] }

/**
 * 십진수 value를 axis('lat'|'lng') 반구 기호와 함께 format('dd'|'dms'|'ddm') 문자열로 만든다.
 * 예: formatCoordinate(37.5665, 'dms', 'lat') → "N37°33'59.4\""
 */
export function formatCoordinate(value, format, axis) {
  const hemi = HEMI[axis][value >= 0 ? 0 : 1]
  const abs = Math.abs(value)
  if (format === 'dd') return `${hemi}${abs.toFixed(5)}°`
  const deg = Math.floor(abs)
  const minFull = (abs - deg) * 60
  if (format === 'ddm') return `${hemi}${deg}°${minFull.toFixed(3)}'`
  const min = Math.floor(minFull)
  const sec = (minFull - min) * 60
  return `${hemi}${deg}°${String(min).padStart(2, '0')}'${sec.toFixed(1).padStart(4, '0')}"`
}
