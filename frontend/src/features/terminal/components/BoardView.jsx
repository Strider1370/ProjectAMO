...
export function BoardView({ activeFlights, pendingFlights, transition, currentPage, pageCount, motionMode, onReplay, onSelectMotion, onSelectView }) {
  return <section className={`exact-screen exact-board motion-${motionMode}`} data-testid="option-one">
    ...
    <div className="board-viewport">
      <div className={`board-page ${transition ? 'is-leaving' : ''}`}><BoardContents flights={activeFlights} /></div>
      {transition && <div className="board-page is-entering" aria-hidden="true"><BoardContents flights={pendingFlights} /></div>}
    </div>
