...
test('상단 날짜·시각은 화면의 공통 서체로 맞추고 날짜도 충분히 크게 표시한다', () => {
  assert.match(styles, /\.board-header-clock span \{[^}]*font-family: inherit[^}]*font-size: 24px/)
  assert.match(styles, /\.board-header-clock strong \{[^}]*font-family: inherit[^}]*font-size: 36px/)
  assert.match(styles, /\.rail-header-clock span \{[^}]*font-family: inherit[^}]*font-size: 24px/)
})
...
test('도착 현지 시간 기준 안내문은 원거리에서도 읽을 수 있는 크기로 표시한다', () => {
  assert.match(styles, /\.screen-footer \{[^}]*font-size: 16px/)
...
test('기관 안내는 하단 우측 QR로, 페이지 표시는 하단 중앙으로 배치한다', () => {
  ...
  assert.match(styles, /\.page-indicator \{[^}]*top: auto[^}]*bottom: 17px/)
  ...
  assert.match(styles, /\.screen-footer \{[^}]*padding: 0 24px 0 56px/)
...
test('1안은 3안과 같은 헤더 중앙 위치에 시험용 전환 컨트롤을 두고 기관 워드마크를 하단 QR 왼쪽에 둔다', () => {
  ...
  assert.match(styles, /\.exact-board \.board-header-actions \{[^}]*top: 22px[^}]*left: 66%/)
  assert.match(styles, /\.screen-footer \.header-weather-panel \{[^}]*grid-template-columns: 110px auto 50px/)
})

test('3안의 한국 시각은 1안처럼 헤더 맨 오른쪽에 고정되고 조작 콘솔과 분리된다', () => {
  assert.match(styles, /\.rail-header-clock \{[^}]*position: absolute[^}]*top: auto[^}]*right: 42px[^}]*bottom: 27px[^}]*left: auto/)
...
test('3안 왼쪽 패널은 1안과 같은 원거리 판독 위계로 도시·현지 시각·항공편·운항 값을 표시한다', () => {
  assert.match(styles, /\.rail-flight-row \{[^}]*grid-template-columns: 33% 67%/)
  assert.match(styles, /\.rail-flight-info h2 \{[^}]*font-size: 59px/)
  assert.match(styles, /\.rail-local-clock strong \{[^}]*font-size: 34px/)
  assert.match(styles, /\.rail-flight-number strong \{[^}]*font-size: 44px/)
  assert.match(styles, /\.rail-flight-status > span \{[^}]*font-size: 25px/)
  assert.match(styles, /\.rail-stats span \{[^}]*font-size: 20px/)
  assert.match(styles, /\.rail-stats strong \{[^}]*font-size: 44px/)
  assert.match(styles, /\.rail-stats > div \{[^}]*row-gap: 8px/)
})
...
test('1안은 원거리 판독이 필요한 상태·현지 시각·운항 label을 확대한다', () => {
  assert.match(styles, /\.operation-status strong \{[^}]*font-size: 25px/)
  assert.match(styles, /\.local-clock-main strong \{[^}]*font-size: 34px/)
...
test('1안은 확대된 글자 주변에 읽기 여백을 유지한다', () => {
  assert.match(styles, /\.airline-flight-meta \{[^}]*row-gap: 12px/)
  assert.match(styles, /\.schedule-grid > div \{[^}]*row-gap: 12px/)
  assert.match(styles, /\.operation-status \{[^}]*gap: 12px/)
  ...
  assert.match(styles, /\.board-column \{[^}]*grid-template-rows: 140px 120px 140px 238px 232px/)
})
