  ...
  return (
    <section className="my-map-editor-toolbar" aria-label="지도 작성 도구">
      ...
      <div className="my-map-editor-options">
        <label className="my-map-editor-check"><input type="checkbox" checked={editor.continuousPoint} disabled={editor.activeTool !== 'point'} onChange={(event) => onAction('setContinuousPoint', event.target.checked)} /> 지점 연속 추가</label>
        <label>추가할 그룹
      ...
      {drawing && <div className="my-map-draft-hint" role="status">
        <span>{editor.activeTool === 'circle' ? (editor.draft?.coordinates?.length ? '원의 반경을 정할 위치를 누르세요.' : '원의 중심을 누르세요.') : `${TOOLS.find((tool) => tool.id === editor.activeTool)?.label} · ${editor.draft?.coordinates?.length ?? 0}개 점 입력됨`}</span>
        <div>
          ...
          {(editor.activeTool === 'line' || editor.activeTool === 'polygon') && <button type="button" className="my-map-primary-button" onClick={() => onAction('finishDraft')}>도형 완료</button>}
