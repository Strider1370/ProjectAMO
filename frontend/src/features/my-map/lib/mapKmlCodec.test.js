...
import { circleGeometry } from './mapDocument.js'
...
function fixture() {
  return {
    ...
    createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T01:00:00.000Z', ungroupedOrder: 3,
...
test('그룹 없는 항목 블록의 위치는 전체·부분 내보내기와 개인 사본에서 유지된다', async () => {
  ...
  assert.equal((await importMapDocument(toBuffer(partial), 'partial.kml')).ungroupedOrder, 0)
