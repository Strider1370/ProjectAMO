...
const MODEL_FIXTURE = {
  nwp_ecmwf: { status: 'ok', modelRunAt: null, airportRuns: [{ airportIcao: 'RKSI', modelRunAt: '2026-09-06T00:00:00.000Z' }, { airportIcao: 'RKPU', modelRunAt: '2026-09-06T06:00:00.000Z' }], availableAt: '2026-09-06T07:09:00.000Z', collectedAt: '2026-09-06T07:20:00.000Z', successAirports: 7, failedAirports: 1, nextCheckAt: '2026-09-06T08:40:00.000Z', lastFailure: { airportIcao: 'RKSS', code: 'provider_failed', message: 'provider request failed' } },
  nwp_gfs: { status: 'late', modelRunAt: '2026-09-06T00:00:00.000Z', airportRuns: [{ airportIcao: 'RKSI', modelRunAt: '2026-09-06T00:00:00.000Z' }], availableAt: '2026-09-06T05:36:00.000Z', collectedAt: '2026-09-06T05:48:00.000Z', successAirports: 8, failedAirports: 0, nextCheckAt: '2026-09-06T08:10:00.000Z', lastFailure: null },
...
export async function prepareAdminAccounts() {
  const backendDir = path.resolve(import.meta.dirname, '..', '..', 'backend')
  const [{ getDb }, { createUser }] = await Promise.all([
    import(pathToFileURL(path.join(backendDir, 'src/db/index.js')).href),
    import(pathToFileURL(path.join(backendDir, 'src/db/users.js')).href),
  ])
  const db = getDb()
  for (const account of ACCOUNTS) {
    try {
      ...
      if (error.message === 'username_taken') continue
      throw error
    }
  }
  return { dataPath: process.env.DATA_PATH, accounts: ACCOUNTS.map(({ username, role, status }) => ({ username, role, status })) }
}
