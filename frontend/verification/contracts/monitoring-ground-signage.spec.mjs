...
test.describe('ground-signage', () => {
  ...
  test('ground-signage preserves forecast extremes and date-boundary emphasis', async ({ page }) => {
    ...
    await expect(extremes.filter({ hasText: '30°' })).toHaveClass(/is-max/)
    await expect(extremes.filter({ hasText: '23°' })).toHaveClass(/is-min/)
    await expect(extremes.filter({ hasText: '30°' })).toHaveCSS('fill', 'rgb(220, 38, 38)')
    await expect(extremes.filter({ hasText: '23°' })).toHaveCSS('fill', 'rgb(37, 99, 235)')
    await expect(hourly.locator('[data-hourly-time]').first()).toHaveClass(/is-now/)
    await expect(hourly.locator('[data-hourly-time]').first()).toHaveCSS('fill', 'rgb(21, 128, 61)')
    await expect(hourly.locator('[data-hourly-time].is-daybreak')).toHaveText(/\d+일/)

    const forecast = page.getByRole('region', { name: '지상 예보' })
    await page.clock.runFor(12_000)
    const weekly = forecast.locator('[data-forecast-view="weekly"]')
    await expect(weekly.locator('.ground-forecast-temp-min').first()).toHaveCSS('color', 'rgb(13, 127, 111)')
