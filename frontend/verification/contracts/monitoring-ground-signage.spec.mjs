...
test.describe('ground-signage', () => {
  ...
  test('ground-signage enlarges alert/current values and shows six labelled metric icons', async ({ page }) => {
    ...
    const alert = page.getByRole('region', { name: '공항경보' })
    const current = page.getByRole('region', { name: '현재 날씨' })
    closeTo(await fontSize(alert.locator('[data-warning-label]')), 30)
    closeTo(await fontSize(alert.locator('[data-warning-time]')), 20)
    closeTo(await fontSize(current.locator('[data-current-temperature]')), 64)
    closeTo(await fontSize(current.locator('[data-current-feels]')), 26)
    closeTo(await fontSize(current.locator('[data-current-condition]')), 26)
    closeTo(await fontSize(current.locator('[data-current-metric-value]').first()), 36)
    closeTo(await fontSize(current.locator('[data-current-metric-label]').first()), 20)
